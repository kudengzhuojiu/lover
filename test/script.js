...
function petals() { const p = document.createElement('i'); p.className='petal'; p.textContent=Math.random()>.5?'❤️':'✦'; p.style.left=Math.random()*100+'vw'; p.style.setProperty('--drift',(Math.random()*180-90)+'px'); p.style.animationDuration=(6+Math.random()*6)+'s'; $('#petals').append(p); setTimeout(()=>p.remove(),12500); } setInterval(petals,680);
